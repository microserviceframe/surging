...
namespace Surging.Core.Consul.WatcherProvider.Implementation
{
    class NodeMonitorWatcher : WatcherBase
    {
        ...
        public NodeMonitorWatcher(Func<ValueTask<ConsulClient>> clientCall, IClientWatchManager manager, string path, Action<byte[], byte[]> action, Func<string, bool> allowChange)
        {
            _action = action;
            _manager = manager;
